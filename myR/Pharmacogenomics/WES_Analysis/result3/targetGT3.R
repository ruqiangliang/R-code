...
targetGT <- NULL
snp <- dir('.','*.csv.0')
baitGenes <- read.csv('./allGenes.csv',header=T)
baitGenes <- unique(baitGenes$x)
for (i in 1:length(snp)){
  snpF <- read.csv(snp[i],header=T)
  ID <- strsplit(strsplit(snp[i],split='_')[[1]][3],split='.csv.0')[[1]]
...
for (i in 1:length(snp)){
  snpF <- read.csv(snp[i],header=T)
  ID <- strsplit(strsplit(snp[i],split='_')[[1]][3],split='.csv.1')[[1]]
  GT <- subset(snpF, Gene %in% as.character(baitGenes))
  targetGT <- rbind(targetGT, cbind(rep(ID,length(GT$Gene)),GT))
}
...
for (i in 1:length(baitGenes)){
  if (! grepl(',',as.character(baitGenes[i]))) {
  ...
  if(length(res$ID) !=0){
    ...
    theNum[i]<- matrixCol
    print(c(i, baitGenes[i], theP[i], theNum[i]),quote=F)
    if (theP[i] < 0.25) {
      colnames(theGT)<-pos
      write.csv(cbind(phenoT,theGT),paste('./resultAll/',baitGenes[i],'.csv',sep=''))
    }
  }
}
}
...
write.csv(theReport,'./resultAll/SKAT-All-report.csv')