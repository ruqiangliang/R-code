...
for (i in 1:length(baitGenes)){
  ...
  for (n in 1:matrixCol){
    theGT[,(n*2-1):(n*2)] <- rep(toupper(subset(res,Position == pos [n],
                                                select='Reference.Base')[[1]][1]),2)
  }
  for (j in 1:12){
    theID <- phenoT[j,2]
    ...
    for (m in 1:length(thePos)){
      theGT[j,(2*thePos[m]-1):(2*thePos[m])] <- strsplit(as.character(snpGT[m,3])
                                                         ,split='/')[[1]]
    }
  }
