...
allIHC <- read.csv('./AllRibbonSynapse20130923.csv',header=T)
distr <- subset(allIHC,Noise == 90 | Noise == 96 | Noise == '96dBZO'| Noise == 0)
...
ribAbneuralPerPlot <- ggplot(RibAbneuralPerSE, aes(x=Frequency, y=RibbonAbneuralPercent, color=Noise)) +
  ...
  xlab('Frequency (kHz)') +
  ylab('abneural ribbon (%)') +
  scale_colour_hue(labels=c('No noise','90dB','96dB', '96dB/ZNS'))+
  theme_bw() +
