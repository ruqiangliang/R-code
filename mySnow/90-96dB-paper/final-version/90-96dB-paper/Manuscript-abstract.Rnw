...
\caption{{\bf {Auditory-evoked brainstem response (ABR) threshold tested after 1 day (a) and 2 weeks (b) of noise exposure at different levels and zonisamide treatment.}} Filled square over solid line, 90 dB (n=\Sexpr{format(with(ttsSE,N[Noise==90]))}); filled circle over dashed line, 96 dB (n=\Sexpr{format(with(ttsSE,N[Noise==96]))}); filled triangle over dotted line, administration of zonisamide at 120 mg/kg body weight right before 96 dB noise exposure (n=\Sexpr{format(with(ttsSE,N[Noise=='96ZO']))}). Noise was octave band from 8 to 16 kHz. All values are mean $\pm$ s.e.m.}
\label{fig:Figure1}
\end{figure}
...
<<DPOAE-PTS-IO>>
@
\caption{{\bf {Distortion product otoacoustic emission (DPOAE) tested after noise exposure at different levels.}} A) DPOAE threshold shift at frequency across the 5 kHz to 40 Hz. $L1$ and $L2$ were 75 and 65 dB SPL, respecitvely. Filled square over solid line, 90 dB (n=\Sexpr{max(with(DPOAESE,N[Noise==90]))}); filled circle over dashed line, 96 dB (n=\Sexpr{max(with(DPOAESE,N[Noise==96]))}); filled triangle over dotted line, prophylatical administration of 120 mg/kg zonisamide before 93 dB (n=\Sexpr{max(with(DPOAESE,N[Noise=='96ZO']))}). B) DPOAE growth curve at levels of stimulation up to 80 dB SPL at 28.3 kHz. Black square, without noise exposure (n=\Sexpr{max(with(DPOAESE,N[Noise==0]))}). All values were  mean $\pm$ s.e.m. }
\label{fig:Figure3}
\end{figure}
...
@
\caption{{\bf {Average number of ribbon synapse (A) and postsynaptic receptor patches (B) per inner hair cell was restored by prophylatical administration of zonisamide.}}  Filled and open squares over solid lines, synapse and receptors without noise exposure (n=\Sexpr{length(with(Synapse, unique(Animal[Noise == 0])))}); filled  and open circles over dashed lines, synapse and receptors after noise exposure at 90 dB (n=\Sexpr{length(with(Synapse, unique(Animal[Noise == 90])))}); filled  and open triangles over dotted lines, synapse and receptors after noise exposure at 96 dB (n=\Sexpr{length(with(Synapse, unique(Animal[Noise == 96])))}); filled  and open diamonds over dotdash lines, synapse and receptors of zonisamide treated before noise exposure at 96 dB (n=\Sexpr{length(with(Synapse, unique(Animal[Noise == '96ZO'])))}). Average ($\pm$ s.e.m.) numbers were computed from image analysis of Z-stacks by confocal microscopy scanning at 5, 10, 20, 28.3, and 40 kHz regions.}
\label{fig:Figure5}
\end{figure}
